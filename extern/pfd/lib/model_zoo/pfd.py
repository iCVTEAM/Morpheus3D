...
from extern.pfd.lib.model_zoo.common.get_model import get_model, register
from extern.pfd.lib.log_service import print_log
...
class PromptFreeDiffusion(nn.Module):
    ...
    def vae_encode_trainable(self, x, which, **kwargs):
        z = self.vae[which].encode_trainable(x, **kwargs)
        if self.latent_scale_factor is not None:
            if self.latent_scale_factor.get(which, None) is not None:
                scale = self.latent_scale_factor[which]
                return scale * z
    ...
    def ctx_encode_trainable(self, x, which, **kwargs):
        if which.find('vae_') == 0:
            return self.vae[which[4:]].encode_trainable(x, **kwargs)
        else:
            return self.ctx[which].encode(x, **kwargs)
